...
<div class="panel">
  ...
  <div class="panelcontent">
			<div class="top-portion">
				<div class="top_table">
					<table border=1>
						...
						$query="SELECT object_name, COUNT(*) FROM net_diag WHERE (dn like '%RNC%' AND text LIKE '$_REQUEST[text]%') GROUP BY object_name ORDER BY COUNT(*) DESC";
						$result=mysql_query($query);
				...
				<div class="top_table">
					<table border=1>
						...
						//$query="SELECT object_name, dn, COUNT(*) from net_diag WHERE (text LIKE '$_REQUEST[text]%' AND vip_site='Y' AND (object_name LIKE 'DHK%' OR object_name LIKE 'KHU%' OR object_name LIKE 'RAJ%' OR object_name LIKE 'SYL%')) GROUP BY dn";
						 $query_vip="SELECT object_name, dn, count(*) AS 'cnt' from net_diag where (dn like '%RNC%' AND text LIKE '%$_REQUEST[text]%') GROUP BY dn ORDER BY COUNT(*) DESC"; 
						 }	
	...
	<table class="data_table1" width='100%' id="table1">
		...
		if($_REQUEST['object_type']=='rnc'){
			$query1="SELECT text, alarm_number, object_name, dn, alarm_time, cancel_time, sup_info, ack_status FROM net_diag WHERE (dn like '%RNC%' AND text LIKE '$text%') ORDER BY alarm_time";
		}		
		else if($_REQUEST['object_type']=='juniper'){
